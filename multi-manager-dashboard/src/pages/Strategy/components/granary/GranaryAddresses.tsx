...
const GranaryAddresses = ({ vault, strategy }: IGranaryAddressesProps) => {
    ...
    useEffect(() => {
        (async () => {
            if (strategy.dataProviderAddress) {
                ...
                const response = await contract.getUserReserveData(vault.asset, strategy.address);
        ...
        <div className='grid grid-cols-12 col-span-5 bg-white border border-gray-200 mb-8'>
            <div className={`col-span-12 flex flex-col flex-1`}>
                <div className='p-3 text-gray-800 items-center'>
                    <div className='font-bold'>Provider Addresses</div>
                </div>
                {!strategy?.dataProviderAddress &&
...
export default GranaryAddresses;